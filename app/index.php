...
//
  ...
  if (((!is_null($serviceAccommodatorId)) && (!is_null($serviceLocationId))) || (!is_null($friendlyURL))) {
    ...
    if (isset($saslJSON['error'])) {
        ...
        $apiURL = "http://" . $server . "/apptsvc/rest/html/retrieveAugmentedMediaStreamAsHTML"."?embedded=".($embedded ? "true" : "false");
        
        // php_log("API:".$apiURL);
        
        $mediaStreamJSON       = makeApiCall($apiURL);
        $mediaStream = $mediaStreamJSON['html'];
    } else {
        // good url
        ...
        $apiURL = "http://" . $server . "/apptsvc/rest/html/retrieveAugmentedMediaStreamAsHTML?themeId=".$saslJSON['themeId']."&serviceAccommodatorId=" . $serviceAccommodatorId . "&serviceLocationId=" . $serviceLocationId."&embedded=".($embedded ? "true" : "false");
