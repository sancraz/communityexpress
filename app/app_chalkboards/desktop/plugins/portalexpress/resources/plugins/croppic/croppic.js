...
(function (window, document) {
	...
	Croppic.prototype = {
		...
		bindImgUploadControl: function(){
			...
			that.form.find('input[type="file"]').change(function(){
				...
				if ($(this).val() !== "") {
					...
					img.onload = function() {
						if(this.width >= $('#cropContainerModal').width() && this.height >= $('#cropContainerModal').height()){
							...
							com.faralam.registration.showPopup('Error', 'Minimum size of the Image for '+ $('input.view_options:checked').val() +' should be '+$('#cropContainerModal').css('width')+'(w) X '+$('#cropContainerModal').css('height')+'(h)');
							return false;
						}
					};
					img.onerror = function() {
						com.faralam.registration.showPopup('Error', 'Not a valid file: ' + file.type);
						return false;
					};
			...
			$.ajax({
                ...
                type: 'POST'
					...
					if(response.status=='success'){
						...
						/* added by Alamgir */
						window.croppedImageUrl_hack=response.url;
		...
		makeradioActive: function(){
			var that = this;
			var name = 'service_status_options';
			$('.view_options').removeAttr("disabled");
		}
	};
})(window, document);
